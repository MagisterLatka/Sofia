    ...
    with open(filepath, 'wb') as f:
        ...
        if total is None:
            ...
            for data in response.iter_content(chunk_size=max(int(total/1000), 1024*1024)):
                ...
                elapsedTime = max(time.time() - startTime, 0.00001)
...
def UnzipFile(filepath, deleteZipFile=True):
    ...
    with tarfile.open(zipFilePath, 'r') as zipFileFolder:
        for name in zipFileFolder.getmembers():
            zipFileContent[name.name] = name.size
        ...
        for zippedFileName, zippedFileSize in zipFileContent.items():
            ...
            if os.path.isfile(UnzippedFilePath):
                ...
                zipFileFolder.extract(zippedFileName, path=zipFileLocation)
