...
namespace Sofia {
	...
	void DX11RenderPass::Clear(const glm::vec4& clearVal, float depth, uint8_t stencil) noexcept
	{
		for (uint32_t i = 0; i < D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT; ++i)
		{
			if (m_RenderTargets[i]) m_RenderTargets[i]->Clear(clearVal);
		}
		if (m_DepthStencilTarget) m_DepthStencilTarget->Clear(glm::vec4(0.0f), depth, stencil);
	}
	...
	void DX11RenderPass::Resize(uint32_t width, uint32_t height)
	{
		...
		for (uint32_t i = 0; i < D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT; ++i)
		{
			if (m_RenderTargets[i]) m_RenderTargets[i]->Resize(m_Width, m_Height);
		}
		if (m_DepthStencilTarget) m_DepthStencilTarget->Resize(m_Width, m_Height);
	}
}
