...
std::string DxgiInfoManager::GetMessages() const
{
	...
	for (auto i = m_Next; i < end; ++i)
	{
		...
		SOF_DX_GRAPHICS_CALL(m_DxgiInfoQueue->GetMessageA(DXGI_DEBUG_ALL, i, nullptr, &messageLength));

		auto bytes = std::make_unique<byte[]>(messageLength);
		auto message = reinterpret_cast<DXGI_INFO_QUEUE_MESSAGE*>(bytes.get());

		SOF_DX_GRAPHICS_CALL(m_DxgiInfoQueue->GetMessageA(DXGI_DEBUG_ALL, i, message, &messageLength));
