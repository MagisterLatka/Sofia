...
namespace Sofia {
	...
	void ShaderLibrary::Load(const std::filesystem::path& vertexPath, const std::filesystem::path& fragmentPath)
	{
	...
	void ShaderLibrary::Load(const std::filesystem::path& path)
	{
	...
	void ShaderLibrary::Load(const std::string& name, const std::filesystem::path& vertexPath, const std::filesystem::path& fragmentPath)
	{
		SOF_CORE_ASSERT(m_Shaders.find(name) == m_Shaders.end(), "Shader {0} already exist", name);
		Ref<Shader> shader = Shader::Create(vertexPath, fragmentPath);
		m_Shaders[name] = shader;
	}
	void ShaderLibrary::Load(const std::string& name, const std::filesystem::path& path)
	{
		SOF_CORE_ASSERT(m_Shaders.find(name) == m_Shaders.end(), "Shader {0} already exist", name);
		Ref<Shader> shader = Shader::Create(path);
		m_Shaders[name] = shader;
	}
	void ShaderLibrary::Load(const std::string& name, const std::string& vertexSource, const std::string& fragmentSource)
	{
		SOF_CORE_ASSERT(m_Shaders.find(name) == m_Shaders.end(), "Shader {0} already exist", name);
